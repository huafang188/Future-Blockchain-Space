/**
 * Future Blockchain Space - 逻辑控制中心 (正式终极兼容版)
 * 适配：Bitget, TP, MetaMask, OKX Wallet
 */
...
function getSafeAddr(addr) {
    if (!addr) return "";
    try {
        // 先清理字符串，再进行 Checksum 转换
        return ethers.getAddress(addr.toLowerCase().trim());
    } catch (e) {
        console.error("地址校验失败，请检查地址是否正确:", addr);
        return addr;
    }
}
...
async function connectWallet() {
    if (!window.ethereum) return alert("请在 Web3 钱包浏览器内打开");
...
async function fetchUserData(address) {
    try {
        ...
        console.warn("数据拉取失败，但不影响支付操作:", err);
        renderTokens({}); // 后端崩溃时防止界面留空
    }
}
...
async function handleContractPay(businessType) {
    ...
    if(businessType === 'MINER') {
        ...
        rawAmount = buyTotalEl ? buyTotalEl.innerText.replace('$ ', '') : "0";
